...
const promptButtons = [
    ...
    document.getElementById("prompt-button-4"),
];
...
const DEFAULT_PROMPTS = [
    "Respond using gen-z slang",
    "Respond like a 5 years old kid",
    "Respond like an expert",
...
function resetChatHistory() {
    ...
    addMessage(
        "bot",
        "Hello! I'm Jon-33 powered by Cohere. How can I help you today?"
    );
}
...
promptButtons.forEach((button, index) => {
    ...
    // Right click Prompt 4 to open form with fields to change prompt content
    if (index === 3) {
    ...
    button.addEventListener("click", (e) => {
        //Send Customized tempatures and user_message to frontend depending on prompts button clicked
        if (e.button === 0 && prompts[index]) {
            if (index === 0) {
                temperature = 0.7;
                user_message =
                    "Forget any prior prompts. Respond like a Gen Z: short, slang-heavy replies using rizz, sus, etc. 1–2 lines max.";
            } else if (index === 1) {
                temperature = 0.3;
                user_message =
                    "Forget any prior prompts. Respond like a 5-year-old. Bad grammar, silly tone, very short.";
            } else if (index === 2) {
                temperature = 1;
                user_message =
                    "Forget prior prompts. Respond with expert-level detail, in first person.";
            } else if (index === 3) {
                temperature = 0.8;
                user_message = modalInput;
            }
